...
Dialog::Dialog(QWidget *parent)
    ...
    , requestLineEdit(new QLineEdit(tr("0")))
    , trafficLabel(new QLabel(tr("No traffic.")))
    , statusLabel(new QLabel(tr("Status: Not running.")))
    , runButton(new QPushButton(tr("Red LED on/off")))
{
 ...
 // set waitting time at 500ms
    waitResponseSpinBox->setValue(500);

 // add picture

    QPixmap pix("logo.png");
    serialPortLabel->setPixmap(pix);
  ...
  //  mainLayout->addWidget(statusLabel, 4, 0, 1, 5);
    ...
    setWindowTitle(tr("LaunchPad QILP"));
